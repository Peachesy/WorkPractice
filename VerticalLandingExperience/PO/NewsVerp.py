...
class NewsVerp(Base):
    ...
    def get_current_url(self):
        ...
        return self.verp_url

    def get_text(self, market_list):

        try:
            self.find(self.search_box_value_ele)
        except NoSuchElementException:
            self.no_search_text_market.append(market_list)
        else:
            self.search_box_text = self.find(self.search_box_value_ele).get_attribute("value")

        return self.search_box_text

    # Check if category bar exist and if top stories button selected
    def check_categroybar(self,market):
        try:
        ...
        return self.no_category_bar_market,self.did_not_select_top_stories_market

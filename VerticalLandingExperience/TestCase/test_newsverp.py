...
class TestNewsVerp(Base):
    nv = NewsVerp()
    hp = BingHP()

    # 1. if the categroy bar exist and the top stories selected
    def test_category(self, market):
        for i in market:
            self.hp.go_to_verp(i).check_categroybar(i)
    ...
    # # 3.If the texts of the 3 points are the same
    # def test_3texts_same(self):
